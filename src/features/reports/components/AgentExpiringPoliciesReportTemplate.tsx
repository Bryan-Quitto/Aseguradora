...
interface AgentExpiringPoliciesReportTemplateProps {
  id: string;
  reportTitle: string;
...
const AgentExpiringPoliciesReportTemplate = ({ id, reportTitle, policies, generatedAt, daysThreshold, agentName }: AgentExpiringPoliciesReportTemplateProps) => {
    ...
    <div id={id} className="p-8 bg-white text-gray-900 font-sans text-sm">
      <header className="flex justify-between items-center pb-4 mb-6 border-b-2 border-gray-800">
        <div>
          ...
          <p className="text-gray-600 mt-1">Reporte Personal de Renovaciones</p>
        </div>
                ...
                {policies.map((policy) => {
                  ...
                  return (
                    <tr key={policy.id} className="hover:bg-gray-50">
                      ...
                      <td className={`border p-2 text-center font-bold ${ daysLeft <= 7 ? 'text-red-600' : daysLeft <= 30 ? 'text-yellow-600' : 'text-gray-700' }`}>
      ...
      <footer className="mt-12 pt-4 border-t text-center text-xs text-gray-500">
        <p>Este es un documento generado para la gestión proactiva de renovaciones.</p>
...
export default AgentExpiringPoliciesReportTemplate;