import React, { useState, useEffect, ChangeEvent, FormEvent, useMemo } from 'react';
 ...
     const { coverage_amount, ad_d_included, ad_d_coverage, age_at_inscription } = formData;

     // Lógica de cálculo de la prima (ejemplo básico, ajustar según tus reglas de negocio)
     let basePremium = coverage_amount ? coverage_amount * 0.002 : 0; // $2 por cada $1000 de cobertura

     if (ad_d_included && ad_d_coverage) {
       basePremium += ad_d_coverage * 0.001; // $1 por cada $1000 de cobertura AD&D
     }

     // Ajuste por edad (ejemplo)
     if (age_at_inscription && age_at_inscription > 40) {
       basePremium *= 1 + (age_at_inscription - 40) * 0.01; // Incremento del 1% por cada año > 40
     }

     // Asegurar prima mínima
     return Math.max(20, basePremium);
   }, [formData.coverage_amount, formData.ad_d_included, formData.ad_d_coverage, formData.age_at_inscription]);
 ...
       } else if (['premium_amount', 'coverage_amount', 'ad_d_coverage'].includes(name)) {
 ...
         start_date: '',
         end_date: '',
         premium_amount: 20, // Resetear a la prima mínima inicial
         payment_frequency: 'monthly',
         status: 'pending',
         contract_details: '',
 ...
               step="0.01"
               className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
               readOnly // Este campo ahora es de solo lectura, ya que el valor se calcula
 ...
         {/* NUEVO: Prima Estimada / Validación */}
