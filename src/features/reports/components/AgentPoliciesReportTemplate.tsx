...
interface AgentPoliciesReportTemplateProps {
  id: string;
  reportTitle: string;
...
const AgentPoliciesReportTemplate = ({ id, reportTitle, policies, generatedAt, agentName }: AgentPoliciesReportTemplateProps) => {
    ...
    <div id={id} className="p-8 bg-white text-gray-900 font-sans text-sm">
      <header className="flex justify-between items-center pb-4 mb-6 border-b-2 border-gray-800">
        <div>
          ...
          <p className="text-gray-600 mt-1">Reporte Personal de Agente</p>
        </div>
      ...
      <main>
        ...
        <section className="mb-8 grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="p-4 bg-gray-100 rounded-lg text-center"><p className="text-xs font-bold">Total Pólizas</p><p className="text-2xl font-extrabold">{totalPolicies}</p></div>
            <div className="p-4 bg-green-50 rounded-lg text-center"><p className="text-xs font-bold">Pólizas Activas</p><p className="text-2xl font-extrabold">{activePolicies}</p></div>
            <div className="p-4 bg-yellow-50 rounded-lg text-center"><p className="text-xs font-bold">Pólizas Pendientes</p><p className="text-2xl font-extrabold">{pendingPolicies}</p></div>
            <div className="p-4 bg-blue-50 rounded-lg text-center"><p className="text-xs font-bold">Suma de Primas (Activas)</p><p className="text-2xl font-extrabold">{formatCurrency(totalPremiumSum)}</p></div>
        </section>

        <section>
          <h3 className="text-lg font-bold border-b pb-2 mb-4 text-blue-800">Listado de Pólizas Gestionadas</h3>
                ...
                {policies.map((policy) => (
                  <tr key={policy.id} className="hover:bg-gray-50">
                    ...
                    <td className="border p-2 text-center">
                        <span className={`px-2 py-1 rounded-full text-white text-[10px] ${ policy.status === 'active' ? 'bg-green-500' : policy.status === 'pending' ? 'bg-yellow-500' : policy.status === 'awaiting_signature' ? 'bg-blue-500' : policy.status === 'expired' ? 'bg-red-500' : 'bg-gray-500' }`}>
      ...
      <footer className="mt-12 pt-4 border-t text-center text-xs text-gray-500">
        <p>Reporte personal de pólizas gestionadas.</p>
...
export default AgentPoliciesReportTemplate;