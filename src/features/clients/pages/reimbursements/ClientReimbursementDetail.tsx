...
// --- AÑADIMOS LAS ETIQUETAS PARA LAS RAZONES DE RECHAZO ---
const rejectionReasonLabels: { [key: string]: string } = {
    illegible_document: 'Documento(s) Ilegible(s)',
    missing_document: 'Falta(n) Documento(s)',
    inconsistent_data: 'Información Inconsistente',
    out_of_coverage: 'Servicio Fuera de Cobertura',
    policy_not_active: 'Póliza no activa en la fecha',
...
export default function ClientReimbursementDetail() {
    ...
    const fetchAllDetails = useCallback(async () => {
        ...
        try {
            ...
            if (requestError || !requestData) {
                throw new Error('No se pudo cargar tu solicitud.');
            }
    ...
    if (accessDenied) {
        return <Navigate to="/client/dashboard/reimbursements" replace />;
    }
                ...
                return (
                    <div className="bg-red-50 p-6 rounded-lg shadow-sm border border-red-200">
                        ...
                        <Link
                            ...
                            className="w-full text-center block bg-blue-600 text-white font-bold py-3 px-4 rounded-lg hover:bg-blue-700 transition"
                        >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="bg-gray-50 p-6 rounded-lg shadow-sm">
                    ...
                    <p className="mt-2"><strong>Estado:</strong>
                        <span className={`ml-2 px-2 inline-flex text-sm leading-5 font-semibold rounded-full ${getStatusStyles(request.status)}`}>
                        ...
                        <ul className="bg-white rounded-lg p-4 border space-y-2">
                            {submittedDocs.map(doc => (
                                <li key={doc.id} className="flex justify-between items-center py-2 border-b last:border-b-0">
                                    ...
                                    <a href={doc.file_url} target="_blank" rel="noopener noreferrer" className="bg-indigo-500 text-white font-bold py-1 px-3 rounded-full text-sm hover:bg-indigo-600">
                                        Ver
                                    </a>
                                </li>
                            ))}
                        </ul>
                    ) : <p>No se encontraron documentos para esta solicitud.</p>}
                </div>
