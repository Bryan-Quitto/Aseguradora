...
export default function AdminPolicyForm() {
    ...
    if (!user || !user.id) return <div className="text-yellow-500 text-center p-4">No se pudo obtener el ID del usuario. Asegúrese de estar autenticado.</div>;
    ...
    const content = !selectedProduct ? (
        ...
        <div>
            ...
            {selectedProduct.type === 'life' && <AdminGenericLifePolicyForm product={selectedProduct} agentId={user.id} />}
            {selectedProduct.type === 'health' && <AdminGenericHealthPolicyForm product={selectedProduct} agentId={user.id} />}
