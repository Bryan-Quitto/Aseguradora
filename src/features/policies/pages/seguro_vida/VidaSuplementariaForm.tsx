  ...
  const handleChange = (
    ...
    setFormData(prev => {
      ...
      if (name === 'ad_d_included') {
        // *** CORRECCIÓN AQUÍ ***
        // Protección de tipo para asegurar que es un HTMLInputElement y un checkbox
        const isCheckbox = (target: EventTarget): target is HTMLInputElement =>
          (target as HTMLInputElement).type === 'checkbox';
  
        if (isCheckbox(e.target)) {
          // Si se desmarca, asegúrate de que ad_d_coverage se establezca a 0
          return { ...prev, ad_d_included: e.target.checked, ad_d_coverage: e.target.checked ? (prev.ad_d_coverage || 0) : 0 };
        }
        return prev; // Si no es un checkbox, retorna el estado previo
      }
...
const handleBeneficiaryChange = (
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (formData.age_at_inscription! < 18 || formData.age_at_inscription! > 60) { // Usar '!'
      setError('La edad de inscripción debe estar entre 18 y 60 años.');
      return;
    }
    // 2) coverage_amount >= 10000
    if (formData.coverage_amount! < 10000) { // Usar '!'
      setError('La cobertura mínima es $10,000.');
      return;
    }
    // (Si ad_d_included): ad_d_coverage <= 2×coverage_amount
    if (formData.ad_d_included) {
      // Aquí, asume que si ad_d_included es true, ad_d_coverage también existe
      if (formData.ad_d_coverage! < 1 || formData.ad_d_coverage! > formData.coverage_amount! * 2) { // Usar '!'
        setError(
          `Si incluye AD&D, su cobertura debe estar entre $1 y $${(formData.coverage_amount || 0) * 2}.` // Usar || 0 para evitar undefined en el string
        );
        return;
      }
    }
    // 3) premium_amount >= 20 (prima mínima) y backend validará tope máximo
    if (formData.premium_amount! < 20) { // Usar '!'
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div>
          ...
          >
            ...
            {products
              .filter(p => p.name === 'Seguro de Vida Suplementaria')
              .map(product => (
        ...
        {formData.ad_d_included && (
          <div>
            ...
            <input
              ...
              id="ad_d_coverage"
              name="ad_d_coverage"
              value={formData.ad_d_coverage || ''} // Usar || '' para que el input no muestre 'undefined'
              onChange={handleChange}
              required={formData.ad_d_included}
              min={1}
              max={(formData.coverage_amount || 0) * 2} // Usar || 0
              step="1"
              className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
            />
            <p className="mt-1 text-xs text-gray-500">
