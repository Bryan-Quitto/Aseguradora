...
export default function AdminEditPolicy() {
        ...
        const fetchPolicyData = async () => {
            ...
            try {
                ...
                if (policyData.agent_id) {
                    ...
                    if (agentError) throw new Error('Error al cargar el agente.');
                    setAgent(agentData);
                }
    ...
    const handleRejectionChange = (type: 'reason' | 'comment', id: string, value: string | boolean) => {
        setRejectionData(prev => {
            if (type === 'reason') return { ...prev, reasons: { ...prev.reasons, [id]: value as boolean } };
    ...
    const handleConfirmRejection = async () => {
        ...
        try {
            ...
            setTimeout(() => navigate(`/admin/dashboard/policies/${policyId}`), 1500);
        } catch (err: any) {
        ...
        <>
            <div className="bg-white rounded-xl shadow-lg p-8 w-full max-w-4xl border border-blue-100 mx-auto">
                <div className="flex justify-between items-center mb-6">
                    ...
                    <Link to="/admin/dashboard/policies" className="bg-gray-600 text-white px-5 py-2 rounded-lg hover:bg-gray-700">Volver a Pólizas</Link>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-8">
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 text-gray-700">
                        <div className="bg-blue-50 p-6 rounded-lg shadow-sm">
                            <h3 className="text-xl font-semibold text-blue-800 mb-4">Póliza</h3>
                            <p><strong>Número:</strong> {policy.policy_number}</p>
                            <p><strong>Producto:</strong> {product?.name}</p>
                            <p><strong>Tipo:</strong> {product?.type}</p>
                        </div>
                        <div className="bg-teal-50 p-6 rounded-lg shadow-sm">
                            <h3 className="text-xl font-semibold text-teal-800 mb-4">Cliente</h3>
                            {client ? (
                                <>
                        ...
                        <div className="bg-purple-50 p-6 rounded-lg shadow-sm">
                            <h3 className="text-xl font-semibold text-purple-800 mb-4">Agente</h3>
                            {agent ? (
                                <>
                                    <p><strong>Nombre:</strong> {agent.full_name}</p>
                    ...
                    <div className="pt-6 border-t">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4">
                            <div>
                                ...
                                <select id="status" value={status} onChange={(e) => handleStatusChange(e.target.value as Policy['status'])} className="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm">
                            ...
                            <div>
                                <label htmlFor="dateOfBirth" className="block text-sm font-medium text-gray-700">Fecha de Nacimiento del Cliente</label>
                                <input type="date" id="dateOfBirth" value={dateOfBirth} onChange={(e) => setDateOfBirth(e.target.value)} required className="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm" />
                            </div>
                            <div className="md:col-span-2">
                                <label htmlFor="contractDetails" className="block text-sm font-medium text-gray-700">Detalles del Contrato (Notas)</label>
                                <textarea id="contractDetails" value={contractDetails} onChange={(e) => setContractDetails(e.target.value)} rows={4} className="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm" />
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex justify-end space-x-4 pt-6 border-t">
                        <button type="button" onClick={() => navigate(`/admin/dashboard/policies/${policyId}`)} className="px-6 py-3 border rounded-md text-gray-700 bg-white hover:bg-gray-50">Cancelar</button>
                ...
                <div className="fixed inset-0 bg-gray-600 bg-opacity-75 flex justify-center items-center z-50">
                    <div className="bg-white p-8 rounded-lg shadow-2xl w-full max-w-lg mx-4">
                        ...
                        <div className="flex justify-end space-x-4 mt-8 pt-4 border-t">
                            <button onClick={() => { setIsRejectionModalOpen(false); policy && setStatus(policy.status); }} className="px-4 py-2 bg-gray-300 rounded-md hover:bg-gray-400">Cancelar</button>
