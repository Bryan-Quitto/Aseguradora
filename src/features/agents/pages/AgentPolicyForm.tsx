...
export default function AgentPolicyForm() {
    ...
    if (!user || !user.id) return <div className="text-yellow-500 text-center p-4">No se pudo obtener el ID del agente. Asegúrese de estar autenticado.</div>;
    ...
    const content = !selectedProduct ? (
        ...
        <div>
            ...
            {selectedProduct.type === 'life' && <GenericLifePolicyForm product={selectedProduct} agentId={user.id} />}
            {selectedProduct.type === 'health' && <GenericHealthPolicyForm product={selectedProduct} agentId={user.id} />}
            {selectedProduct.type !== 'life' && selectedProduct.type !== 'health' && (
                <div className="text-red-500 text-center">Tipo de producto no soportado: {selectedProduct.type}.</div>
            )}
        </div>
    );
    ...
    return (
        <div className="flex justify-center w-full">
            <div className="w-full max-w-4xl">
                 <PageContainer title="Crear nueva póliza">
                    {content}
                 </PageContainer>
            </div>
        </div>
