...
import { validateUniqueProfile } from 'src/features/admin/hooks/validateUniqueProfile'; // Importa la función
...
export default function CrearUsuarios() {
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    if (['primerNombre', 'segundoNombre', 'primerApellido', 'segundoApellido'].includes(name)) {
      ...
      if (value && (!validateNumber(value) || value.length > 10)) {
        newErrors.numeroID = 'Solo se permiten hasta 10 números';
      } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!/^\d{10}$/.test(formData.numeroID)) {
      newErrors.numeroID = 'Debe tener exactamente 10 números';
    }
    ...
    try {
      // 1. Verificar unicidad antes de crear el usuario en Auth
      const { duplicated } = await validateUniqueProfile(
        '', // user_id aún no existe, pero igual validamos email y numeroID
        formData.email,
        formData.numeroID
      );
      if (duplicated.length > 0) {
        const msg = duplicated
          .map(d => {
            if (d.field === 'numero_identificacion') {
              return `número de identificación: ${d.value}`;
            }
            if (d.field === 'email') {
          ...
          .join(', ');
        setSubmissionMessage(`Ya existe un perfil con ${msg}`);
        setFormData({
          primerNombre: '',
          segundoNombre: '',
          primerApellido: '',
