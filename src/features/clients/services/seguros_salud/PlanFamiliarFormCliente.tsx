import React, { useState, useEffect, ChangeEvent, FormEvent, useCallback } from 'react';
 ...
 export default function PlanFamiliarFormCliente() {
  ...
  start_date: '',
  end_date: '',
  premium_amount: 0, // This will be calculated and updated
  ...
  // Nuevo estado para errores de validación específicos del premio
  const [validationErrors, setValidationErrors] = useState<{ premium_amount?: string }>({});
  ...
  // Effect to calculate premium whenever relevant formData changes
  useEffect(() => {
  const calculatePremium = () => {
  let basePremium = 300; // Base premium as per prompt
  ...
  basePremium += formData.num_dependents * 100;
  ...
  // Validate calculated premium against min/max allowed for submission
  if (finalPremium < 300 || finalPremium > 1200) {
  setValidationErrors(prev => ({
  ...prev,
  premium_amount: 'El monto de la prima calculado está fuera del rango permitido ($300 – $1,200 mensuales).',
  }));
  } else {
  setValidationErrors(prev => {
  const newErrors = { ...prev };
  delete newErrors.premium_amount;
  return newErrors;
  });
  }

  setFormData(prev => ({ ...prev, premium_amount: finalPremium })); // Update formData with calculated premium
  };
  ...
  // Now checking calculated premium against min/max on submit
  if (calculatedPremium < 300 || calculatedPremium > 1200) {
  return 'El monto de la prima calculado está fuera del rango permitido ($300 – $1,200 mensuales).';
  }
  ...
  }, [formData, user?.id, calculatedPremium]); // Added calculatedPremium to dependencies
  ...
  status: 'pending',
  premium_amount: calculatedPremium, // Use the calculated premium for submission
  ...
  {/* Display calculated premium here */}
  {/* MOVED THIS SECTION TO THE BOTTOM */}
  <div className="bg-blue-50 p-4 rounded-md shadow-sm">
  <h3 className="text-lg font-semibold text-blue-700 mb-2">Prima Estimada:</h3>
  <p className="text-2xl font-bold text-blue-900">${calculatedPremium.toFixed(2)} / {formData.payment_frequency === 'monthly' ? 'mes' : formData.payment_frequency === 'quarterly' ? 'trimestre' : 'año'}</p>
